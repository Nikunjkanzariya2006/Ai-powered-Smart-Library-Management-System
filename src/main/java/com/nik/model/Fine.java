...
@Table(
        name = "fines",
        indexes = {
                @Index(name = "idx_fines_book_loan_id", columnList = "book_loan_id"),
                @Index(name = "idx_fines_user_id", columnList = "user_id"),
                @Index(name = "idx_fines_status", columnList = "status"),
                @Index(name = "idx_fines_type", columnList = "type")
        }
)
...
public class Fine {
    ...
    /**
     * Check if the fine is still unpaid.
    ...
    public void applyPayment(Long paymentAmount) {
        if (paymentAmount == null || paymentAmount <= 0) {
            throw new FineException("Payment amount must be positive");
        }
